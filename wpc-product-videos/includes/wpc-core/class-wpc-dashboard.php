...
if ( ! class_exists( 'WPCleverDashboard' ) ) {
    class WPCleverDashboard {
        function __construct() {
            ...
            add_action( 'wp_ajax_wpc_export', [ $this, 'ajax_export' ] );
            add_action( 'wp_ajax_wpc_import', [ $this, 'ajax_import' ] );
        }

        function enqueue_scripts() {
            wp_enqueue_style( 'wpc-dashboard', WPC_CORE_URI . 'assets/css/dashboard.css', [], WPC_CORE_VERSION );
            wp_enqueue_script( 'wpc-dashboard', WPC_CORE_URI . 'assets/js/dashboard.js', [
                    'jquery',
                    'jquery-ui-dialog'
            ], WPC_CORE_VERSION, true );
        ...
        /**
         * Render shared header + nav for all WPClever pages.
         *
         * @param string $active_page Current page slug (e.g. 'about', 'kit', 'keys').
         */
        public static function render_header( $active_page = 'about' ) {
            // Core nav items
            $nav_items = [
                'about' => [
                    'url'   => admin_url( 'admin.php?page=wpclever' ),
                    'label' => 'About',
                ],
                'kit'   => [
                    'url'   => admin_url( 'admin.php?page=wpclever-kit' ),
                    'label' => 'Essential Kit',
                ],
            ];

            /**
             * Filter the WPClever admin nav items.
             *
             * @param array  $nav_items   Associative array of slug => ['url' => '', 'label' => ''].
             * @param string $active_page The current active page slug.
             */
            $nav_items = apply_filters( 'wpc_admin_nav_items', $nav_items, $active_page );
            ?>
            <div class="wpc-header">
                <div class="wpc-header-inner">
                    <div class="wpc-logo">
                        <img src="<?php echo esc_url( WPC_CORE_URI . 'assets/images/wpc-icon.svg' ); ?>"
                             alt="WPClever" width="32" height="32"/>
                    </div>
                    <div>
                        <h1>WPClever | Make clever moves</h1>
                        <p class="wpc-tagline">Must-have plugins for powering up online visibility, sales and profits.</p>
                    </div>
                </div>
            </div>
            <div class="wpc-admin-nav">
                <div class="wpc-nav-container">
                    <?php foreach ( $nav_items as $slug => $item ) : ?>
                        <a href="<?php echo esc_url( $item['url'] ); ?>"
                           class="wpc-nav-item<?php echo $active_page === $slug ? ' active' : ''; ?>"><?php echo esc_html( $item['label'] ); ?></a>
                    <?php endforeach; ?>
                </div>
            </div>
            <?php
        ...
        function admin_menu_content() {
            ...
            <div class="wpclever_page wrap">
                <?php self::render_header( 'about' ); ?>

                <div class="wpc-card">
                    <h2 class="wpc-card-title">
                        <span class="dashicons dashicons-info-outline"></span> About
                    </h2>
                ...
                <div class="wpc-card">
                    <h2 class="wpc-card-title">
                        ...
                        <span class="wpclever_plugins_order">
                            <a href="#" class="wpclever_plugins_order_a" data-o="p">popular</a> |
                            <a href="#" class="wpclever_plugins_order_a" data-o="u">last updated</a> |
                            <a href="#" class="wpclever_plugins_search_btn" title="Search"><span class="dashicons dashicons-search"></span></a>
                        </span>
                        <span class="wpclever_plugins_search" style="display:none">
                            <input type="text" class="wpclever_plugins_search_input" placeholder="Search plugins…" />
                            <a href="#" class="wpclever_plugins_search_close" title="Close">&times;</a>
                        </span>
                    </h2>
